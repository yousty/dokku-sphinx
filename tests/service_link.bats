...
@test "($PLUGIN_COMMAND_PREFIX:link) error when the service is already linked to app" {
  ...
  assert_contains "${lines[*]}" "Already linked as SPHINX_URL"
}

@test "($PLUGIN_COMMAND_PREFIX:link) exports SPHINX_URL to app" {
  dokku "$PLUGIN_COMMAND_PREFIX:link" l my_app
  url=$(dokku config:get my_app SPHINX_URL)
  assert_contains "$url" "sphinx://dokku-sphinx-l:9306"
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" l my_app
}

@test "($PLUGIN_COMMAND_PREFIX:link) generates an alternate config url when SPHINX_URL already in use" {
  dokku config:set my_app SPHINX_URL=sphinx://host:9306
  dokku "$PLUGIN_COMMAND_PREFIX:link" l my_app
  run dokku config my_app
  assert_contains "${lines[*]}" "DOKKU_SPHINX_"
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" l my_app
}
...
@test "($PLUGIN_COMMAND_PREFIX:link) links to app with docker-options" {
  ...
  assert_contains "${lines[*]}" "--link dokku.sphinx.l:dokku-sphinx-l"
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" l my_app
}
